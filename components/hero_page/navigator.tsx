...
const Navigator = ({ isCourse = false }: Props) => {
  ...
  return (
    <nav className="bg-[#F5F8FE] flex items-center justify-between px-6 md:px-12 lg:px-20 py-2 rounded-b-lg relative z-50 w-full max-w-screen-l mx-auto shadow-sm">
      {/* Logo */}
      ...
      >
        <div className="flex flex-col lg:flex-row items-start lg:items-center space-y-6 lg:space-y-0 lg:space-x-8 w-full">
          ...
          {isCourse ? (
            ...
            <Link
              className="block lg:inline-block w-full lg:w-auto text-left lg:text-center bg-[#043873] text-white py-2 px-5 rounded-lg text-md hover:bg-opacity-80 hover:ring-2 hover:ring-offset-2 hover:ring-blue-700 transition"
              href="/"
              onClick={() => setIsOpen(false)}
            >
              Home
